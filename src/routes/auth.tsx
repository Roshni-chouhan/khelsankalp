...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Role sign-in & registration — KhelSankalp SGSITS" },
      {
...
const DESKS: Desk[] = [
  {
    role: "athlete",
    title: "Athlete",
    blurb: "SGSITS students who play for their branch or the institute team.",
    icon: Trophy,
    kind: "student",
    ...
    note: "Athlete dashboards are activated immediately after registering.",
    tone: {
  ...
  },
  {
    ...
    blurb: "Student organisers running fixtures, scoring and match day operations.",
    icon: Users,
    kind: "student",
    idLabel: "SGSITS roll number",
    ...
    emailLabel: "Student email",
    emailPlaceholder: "you@sgsits.ac.in",
    note: "Core team access is activated once the institute sports head approves it.",
    tone: {
  ...
  },
  {
    ...
    blurb: "Faculty in charge of institute sports — signs in with the official faculty email.",
    icon: GraduationCap,
    kind: "faculty",
    ...
    note: "Faculty access is activated once the super admin approves it.",
    tone: {
  ...
  },
  {
    role: "superadmin",
    title: "Super Admin",
    blurb: "Sports council administrator for the full KhelSankalp ecosystem.",
    icon: ShieldCheck,
    kind: "faculty",
    ...
    note: "The first super admin is activated instantly; later requests need approval.",
    tone: {
...
function AuthPage() {
  ...
  async function finishSignIn(accessToken: string, refreshToken: string) {
  ...
  async function handleSignIn(e: React.FormEvent) {
    ...
    if (!id) {
      toast.error(isFaculty ? "Enter your official email." : "Enter your roll number.");
      return;
    }
    setSignInBusy(true);
    try {
      const res = await signInWithIdentifier({ data: { identifier: id, password } });
      if (!res.ok) {
        toast.error(res.error);
        return;
      }
      await finishSignIn(res.accessToken, res.refreshToken);
    } catch {
    ...
    try {
      const res = await registerMember({
        data: {
          ...
          rollNumber: String(form.get("memberId") ?? ""),
          email: String(form.get("email") ?? ""),
          password: String(form.get("password") ?? ""),
          phone: String(form.get("phone") ?? ""),
          year: isFaculty ? designation : year,
          branch,
          role: desk.role,
        },
      });
      ...
      if (res.status === "approved") {
        toast.success(`${desk.title} account created. Sign in to open your dashboard.`);
      } else {
        toast.success(
          desk.role === "core_team"
  ...
  async function handleForgot() {
    const id = identifier.trim();
    if (!id) {
      toast.error("Enter your roll number or official email first.");
      return;
    }
    await requestPasswordReset({ data: { identifier: id, origin: window.location.origin } });
    toast.success("If that account exists, a reset link is on its way by email.");
  }
          ...
          {DESKS.map((d) => {
            ...
            return (
              ...
              >
                ...
                <p className="relative font-semibold">{d.title}</p>
                <p className="relative mt-1 text-xs text-muted-foreground">{d.blurb}</p>
                {active ? (
        ...
        <Tabs defaultValue="signin">
          ...
          <TabsContent value="signin" className="mt-6">
            <Card className="glass-card mx-auto max-w-xl overflow-hidden p-0">
              <div className="gradient-royal px-6 py-4">
                ...
                <p className="text-xs text-primary-foreground/80">{desk.note}</p>
              </div>
              <div className="p-6">
              <form className="space-y-4" onSubmit={handleSignIn}>
                ...
                <div className="space-y-2">
                  <Label htmlFor="identifier">
                    {isFaculty ? desk.emailLabel : desk.idLabel}
                  </Label>
                  <Input
                    id="identifier"
                    value={identifier}
                    onChange={(e) => setIdentifier(e.target.value)}
                    placeholder={isFaculty ? desk.emailPlaceholder : desk.idPlaceholder}
                    required
                    maxLength={255}
                  />
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <Button type="submit" disabled={signInBusy} className="gradient-royal text-primary-foreground">
          ...
          <TabsContent value="register" className="mt-6">
            <Card className="glass-card border-t-4 border-t-gold p-6">
              <form className="space-y-4" onSubmit={handleRegister} key={desk.role}>
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="memberId">{desk.idLabel}</Label>
                    <Input
                      id="memberId"
                      name="memberId"
                      required
                      maxLength={30}
                      placeholder={desk.idPlaceholder}
                    />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="email">{desk.emailLabel}</Label>
                    <Input
                      id="email"
                      name="email"
                      type="email"
                  ...
                  <div className="space-y-2">
                    <Label>{isFaculty ? "Department" : "Branch"}</Label>
                  ...
                  <div className="space-y-2">
                    <Label>{isFaculty ? "Designation" : "Year of study"}</Label>
                    {isFaculty ? (
                ...
                <p className="text-xs text-muted-foreground">{desk.note}</p>

                <Button type="submit" disabled={regBusy}>
                  {regBusy ? "Creating account…" : `Create ${desk.title.toLowerCase()} account`}
                </Button>
              </form>
            </Card>
          </TabsContent>
