...
import { AthleteCard, MatchCard, SportCard, TournamentCard } from "@/components/cards/EntityCards";
import {
  COLLEGE,
  announcements,
  athletes,
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "KhelSankalp — SGSITS Indore" },
      {
        property: "og:description",
        content: "The official SGSITS Indore sports tournament ecosystem — live and in one place.",
      },
    ],
  }),
  component: Home,
...
function Home() {
  ...
  const topAthletes = [...athletes].sort((a, b) => b.rating - a.rating).slice(0, 4);
    ...
    <div>
        ...
        <div className="relative mx-auto flex min-h-[720px] max-w-7xl flex-col px-4 pb-6 pt-16 sm:min-h-[760px] sm:pt-24">
          <div className="max-w-2xl">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              >
                <Link to="/explore">
                  <Radio className="mr-2 h-4 w-4" /> Live scores
                </Link>
              </Button>
            </div>
          </div>
          ...
          <div className="mt-auto grid gap-3 pt-12 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { icon: Trophy, title: "Upcoming tournaments", copy: "Fixtures, draws and registrations", to: "/tournaments" as const },
              { icon: UserPlus, title: "Athlete profiles", copy: "Stats, medals and achievements", to: "/athletes" as const },
              { icon: ShieldCheck, title: "Team management", copy: "Squads, coaches and performance", to: "/teams" as const },
              { icon: CalendarCheck, title: "Facility booking", copy: "Courts, grounds and time slots", to: "/facilities" as const },
            ].map((item) => (
              <Link key={item.title} to={item.to} className="group min-h-32 border border-primary-foreground/15 bg-primary/55 p-5 backdrop-blur-xl transition hover:-translate-y-1 hover:border-gold/60 hover:bg-primary/70">
                ...
                <p className="mt-5 font-semibold text-primary-foreground">{item.title}</p>
      ...
      <section className="mx-auto mt-8 max-w-7xl px-4">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard icon={Users} label="Registered athletes" value={`${institutionStats.athletes}+`} hint="Across 9 sports" />
          <StatCard icon={Trophy} label="Tournaments" value={institutionStats.tournaments} hint="This academic session" accent="gold" />
          <StatCard icon={Radio} label="Live right now" value={institutionStats.liveMatches} hint="Ball-by-ball updates" accent="live" />
          <StatCard icon={Medal} label="Medals awarded" value={institutionStats.medalsAwarded} hint="Department championship" accent="primary" />
        </div>
      </section>
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 sm:flex sm:justify-between">
          <div className="min-w-0">
            <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.22em] text-live">
              <span className="h-2 w-2 rounded-full bg-live live-dot" /> Live now
            </p>
            <h2 className="mt-2 text-2xl font-bold text-primary sm:text-3xl">Match centre</h2>
          </div>
          <Button asChild variant="ghost">
            <Link to="/explore">All matches <ArrowRight className="ml-1.5 h-4 w-4" /></Link>
          </Button>
        </div>
      ...
      {/* TOURNAMENTS */}
      <section className="mx-auto mt-20 max-w-7xl px-4">
        <h2 className="text-2xl font-bold text-primary sm:text-3xl">Featured tournaments</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          Registration, fixtures, live scoring, points tables and results — end to end.
        </p>
        <div className="mt-6 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {featured.map((t) => (
      ...
      <section className="mx-auto mt-20 max-w-7xl px-4">
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 sm:flex sm:justify-between">
          <h2 className="min-w-0 text-2xl font-bold text-primary sm:text-3xl"> campus sports</h2>
          <Button asChild variant="ghost">
            <Link to="/sports">View all <ArrowRight className="ml-1.5 h-4 w-4" /></Link>
          </Button>
        </div>
        ...
        <Card className="glass-card overflow-hidden p-0 md:grid md:grid-cols-2">
          ...
          <div className="p-8 sm:p-10">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              From your first departmental trial to a digitally verifiable championship certificate,
               KhelSankalp tracks every match, stat, medal and achievement of your SGSITS career.
            </p>
      ...
      <section className="mx-auto mt-20 max-w-7xl px-4">
        <div className="grid gap-8 lg:grid-cols-[1.6fr_1fr]">
          ...
          <div>
            <h2 className="text-2xl font-bold text-primary sm:text-3xl">Announcements</h2>
            <div className="mt-5 space-y-4">
              {announcements.map((a) => (
                <Card key={a.id} className="glass-card gap-1 p-5">
                  <Badge variant="secondary" className="w-fit">{a.category}</Badge>
                  <p className="mt-1 font-semibold text-primary">{a.title}</p>
      ...
      <section className="mx-auto mt-20 max-w-7xl px-4">
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 sm:flex sm:justify-between">
          <h2 className="min-w-0 text-2xl font-bold text-primary sm:text-3xl">Top rated athletes</h2>
          <Button asChild variant="ghost">
            <Link to="/athletes">Full roster <ArrowRight className="ml-1.5 h-4 w-4" /></Link>
          </Button>
        </div>
      ...
      <section className="mx-auto mt-20 max-w-7xl px-4">
        <Card className="relative overflow-hidden border-0 gradient-royal p-10 text-center">
          ...
          <p className="relative mx-auto mt-3 max-w-xl text-sm text-primary-foreground/85">
            Registrations for the SGSITS Hoops League and Aavartan Badminton Open are open now.
          </p>
          <div className="relative mt-7 flex flex-wrap justify-center gap-3">
            <Button asChild size="lg" className="gradient-gold text-gold-foreground">
            ...
            <Button asChild size="lg" variant="outline" className="border-primary-foreground/40 bg-primary-foreground/10 text-primary-foreground hover:bg-primary-foreground/20 hover:text-primary-foreground">
              <Link to="/campus-map"><MapPin className="mr-2 h-4 w-4" /> Campus sports map</Link>
            </Button>
          </div>
        </Card>
      </section>
      ...
      <section className="mx-auto mt-20 max-w-7xl px-4">
        <div className="flex flex-wrap items-center justify-center gap-6 rounded-2xl border border-border bg-surface px-6 py-8 text-center text-sm text-muted-foreground">
          <span className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-royal" /> Est. {COLLEGE.established}</span>
          <span className="flex items-center gap-2"><MapPin className="h-4 w-4 text-royal" /> {COLLEGE.city}</span>
          <span className="flex items-center gap-2"><Trophy className="h-4 w-4 text-royal" /> {institutionStats.teams} active teams</span>
        </div>
      </section>
    </div>
  );
