...
export const CORE_TEAM_DESIGNATIONS = [
  "Sports Coordinator",
  "Core Commitee Member",
  "Treasurer",
  "Planning & Developement",
  "Media & Records and Public Relations ",
  "Marketing and Sponsorships",
  "Cultural Team",
  "Branch Sports Representative",
] as const;

export type CoreTeamDesignation =
...
const rollSchema = z
  ...
  .regex(
    /^[A-Za-z0-9/-]+$/,
    "Roll number can use letters, numbers, - and / only",
  );
...
const registerSchema = z
  .object({
    ...
    designation: z
      .enum(CORE_TEAM_DESIGNATIONS)
      .optional(),
  })
  .superRefine((data, ctx) => {
    if (data.role === "core_team" && !data.designation) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["designation"],
        message: "Please select a Core Team designation.",
      });
    }
    ...
    if (data.role !== "core_team" && data.designation) {
      ctx.addIssue({
        ...
        path: ["designation"],
        message:
          "Designation is only available for Sports Core Team members.",
      });
    }
  });
...
export const registerMember = createServerFn({
  method: "POST",
})
  ...
  .handler(async ({ data }) => {
    ...
    const { error: profileError } =
      await supabaseAdmin.from("profiles").insert({
        ...
        /*
         * Save designation only for Core Team
         */
        designation:
          data.role === "core_team"
    ...
    /*
     * First Super Admin can be automatically approved.
     */
    ...
    return {
      ...
      /*
       * Return designation so the registration
       * page can immediately know what was saved.
       */
      designation:
        data.role === "core_team"
          ? data.designation ?? null
...
export const signInWithIdentifier = createServerFn({
  method: "POST",
})
  .inputValidator((data: unknown) =>
